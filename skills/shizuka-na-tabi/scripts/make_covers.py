...
Usage:
  python3 make_covers.py <地名>
  # 读取 ~/Downloads/静かな旅_work/<地名>_clips/<key>/封面/frame_XXXs.png
  # 输出到 ~/Downloads/静かな旅_work/📦发布包/<地名>/cover_<key>.png

COVERS 列表由调用方（batch 脚本 或 AI）在运行时传入，格式见 COVER_SCHEMA。
"""
...
from config import clips_dir, pkg_dir
...
.title-mask {{
  ...
  background: radial-gradient(ellipse 80% 24% at 50% 46%,
    rgba(0,0,0,0.58) 0%, rgba(0,0,0,0.20) 55%, rgba(0,0,0,0.0) 100%);
}}
...
def generate_covers(name: str, covers: list[dict]) -> list[Path]:
    """
    name   : 地名，用于定位输出目录（via pkg_dir）
    covers : list of dicts，每条 clip 一个 entry，字段见下：
      {
        "key":       "01_highlight",           # clip 名
        "frame":     "/path/to/frame.png",     # 封面底图（来自 merged.mp4）
        "top_label": "KANAZAWA · WINTER",
        "main_title": "金沢",
        "sub_title":  "Snow came in the night",
        "mood":       ["句1", "句2", "句3"],   # 正好3句，来自文案
        "episode":    "Episode 01 · Highlight",
        "bg_pos":     "center 50%",
        "bg_filter":  "saturate(0.75) brightness(0.85)",
      }
    """
    out_dir = pkg_dir(name)
    results = []
    ...
    with sync_playwright() as p:
        ...
        for cfg in covers:
            html = TEMPLATE.format(
                img=b64(cfg["frame"]),
                bg_pos=cfg.get("bg_pos", "center 50%"),
                bg_filter=cfg.get("bg_filter", "saturate(0.75) brightness(0.85)"),
                top_label=cfg["top_label"],
                main_title=cfg["main_title"],
                sub_title=cfg["sub_title"],
                mood_lines="\n".join(
                    f'<div class="mood-line">{l}</div>' for l in cfg["mood"]
                ),
                episode=cfg["episode"],
            )
            ...
            out = out_dir / f"cover_{cfg['key']}.png"
            page.screenshot(path=str(out), full_page=False)
            print(f"  ✓ cover_{cfg['key']}.png → {out}")
            page.close()
            results.append(out)
        browser.close()
...
if __name__ == "__main__":
    # 直接运行时：python3 make_covers.py <地名>
    # 需要在脚本顶部手动填写 COVERS（或由 AI 生成后注入）
    print("make_covers.py: 请通过 generate_covers(name, covers) 调用，或在 batch 脚本中 import 使用。")
