"""Batch process Hakuba Village Autumn walking video into 4 themed clips."""

import os, re, subprocess

VIDEO  = "/Users/likai/Downloads/hakuba.mp4"
OUT    = "/Users/likai/Downloads/hakuba_clips"
JA_SRT = "/tmp/hakuba_ja.srt"
ZH_SRT = "/tmp/hakuba_zh.srt"

os.makedirs(OUT, exist_ok=True)

# Segments: (start_sec, end_sec)
# Key timestamps:
#   45s   = 0:45   AM6:00 夜明け前の村
#  101s   = 1:41   モルゲンロート（山が赤く染まる）
#  165s   = 2:45   水路・農村
#  230s   = 3:50   踏切・北アルプスの関所
#  392s   = 6:32   白馬駅到着
#  467s   = 7:47   紅葉が見えてきた
#  515s   = 8:35   ロープウェイ乗車中・眼下真っ赤
#  544s   = 9:04   白馬岩岳マウンテンリゾート
#  637s   = 10:37  山頂ハイキング開始
#  765s   = 12:45  リフト降り場到着
#  804s   = 13:24  三段紅葉が現れる（雲が晴れた）
#  900s   = 15:00  ロープウェイで山頂へ戻る
#  935s   = 15:35  ブランコ前の三段紅葉絶景
#  990s   = 16:30  白馬マウンテンハーバー
# 1060s   = 17:40  別の場所へ移動
# 1113s   = 18:33  ねずこの森
# 1190s   = 19:50  紅葉映像集
# 1268s   = 21:08  白馬大橋
# 1281s   = 21:21  エメラルドグリーンの川
# 1400s   = 23:20  川辺の遊歩道
# 1697s   = 28:17  イチョウの木
# 1760s   = 29:20  白馬村の各エリアについて
# 2000s   = 33:20  逆光のすすきと唐松岳
# 2060s   = 34:20  斜陽の通り・お店が閉まっている
# 2119s   = 35:19  薄雲越しに沈む太陽
# 2175s   = 36:15  天神坂・締めへ

CLIPS = {
    "01_highlight": {
        "title": "精華版",
        # Hook: 三段紅葉 → モルゲンロート → エメラルド川 → すすき夕陽 → 日没
        "segments": [
            (990,  1060),   # 16:30-17:40  三段紅葉HOOK・マウンテンハーバー
            (101,  165),    # 1:41-2:45    モルゲンロート・山が赤く
            (1281, 1345),   # 21:21-22:25  白馬大橋・エメラルドグリーンの川
            (2000, 2060),   # 33:20-34:20  すすきと唐松岳・斜陽の通り
            (2119, 2170),   # 35:19-36:10  薄雲越し沈む太陽
        ],
    },
    "02_asa": {
        "title": "早朝の白馬",
        # AM6:00 夜明け → モルゲンロート → 踏切・まち歩き → 紅葉の朝
        "segments": [
            (45,   160),    # 0:45-2:40    AM6:00・月と山・モルゲンロート
            (230,  392),    # 3:50-6:32    踏切・まち歩き開始
            (467,  515),    # 7:47-8:35    紅葉が広がる朝
        ],
    },
    "03_yamaue": {
        "title": "岩岳・三段紅葉",
        # ロープウェイ → 山上ハイキング → 三段紅葉 → カフェ → マウンテンハーバー
        "segments": [
            (515,  545),    # 8:35-9:05    ロープウェイ・眼下が燃えている
            (637,  765),    # 10:37-12:45  山上ハイキング・坂道
            (804,  905),    # 13:24-15:05  三段紅葉出現・カフェ・山頂再び
            (935,  1000),   # 15:35-16:40  ブランコ前の三段紅葉絶景
        ],
    },
    "04_kawa": {
        "title": "川・イチョウ・夕暮れ",
        # ねずこの森 → 白馬大橋・松川 → イチョウ → 夕暮れ
        "segments": [
            (1113, 1190),   # 18:33-19:50  ねずこの森・紅葉
            (1268, 1400),   # 21:08-23:20  白馬大橋・松川・河原
            (1697, 1760),   # 28:17-29:20  イチョウの木
            (2119, 2175),   # 35:19-36:15  夕暮れ・薄雲越しの太陽
        ],
    },
}

# ── SRT helpers ───────────────────────────────────────────
def ts_to_sec(ts):
    h,m,sms = ts.split(':'); s,ms = sms.split(',')
    return int(h)*3600+int(m)*60+int(s)+int(ms)/1000

def sec_to_ts(sec):
    h=int(sec//3600); m=int((sec%3600)//60); s=int(sec%60)
    ms=round((sec-int(sec))*1000)
    return f"{h:02d}:{m:02d}:{s:02d},{ms:03d}"

def parse_srt(path):
    with open(path,encoding='utf-8') as f: content=f.read()
    entries=[]
    for block in re.split(r'\n\s*\n', content.strip()):
        lines=block.strip().splitlines()
        if len(lines)<3: continue
        m=re.match(r'(\S+)\s*-->\s*(\S+)',lines[1])
        if not m: continue
        entries.append({'start':ts_to_sec(m.group(1)),'end':ts_to_sec(m.group(2)),'text':'\n'.join(lines[2:])})
    return entries

def make_bilingual_srt(segments, ja_entries, zh_entries, out_path):
    offsets=[]; t=0
    for s,e in segments:
        offsets.append(t-s); t+=(e-s)

    def retime(entries):
        result=[]
        for entry in entries:
            for i,(cs,ce) in enumerate(segments):
                if entry['end']<=cs or entry['start']>=ce: continue
                ns=max(entry['start'],cs)+offsets[i]
                ne=min(entry['end'],ce)+offsets[i]
                if ne<=ns: continue
                result.append((ns,ne,entry['text']))
        return result

    ja=retime(ja_entries)
    zh_map={}
    for e in retime(zh_entries): zh_map[round(e[0],1)]=e[2]

    out=[]; idx=1
    for s,e,ja_text in ja:
        zh_text=zh_map.get(round(s,1),'')
        text=f"{ja_text}\n{zh_text}" if zh_text else ja_text
        out.append(f"{idx}\n{sec_to_ts(s)} --> {sec_to_ts(e)}\n{text}\n")
        idx+=1

    with open(out_path,'w',encoding='utf-8') as f: f.write('\n'.join(out))
    print(f"  字幕: {idx-1} 条 → {out_path}")

# ── FFmpeg helpers ────────────────────────────────────────
def run(cmd):
    result=subprocess.run(cmd,shell=True,capture_output=True,text=True)
    if result.returncode!=0: print(f"  ERR: {result.stderr[-200:]}")
    return result.returncode==0

def cut_and_merge(segments, merged_path, clip_dir):
    parts=[]
    for i,(s,e) in enumerate(segments):
        p=f"{clip_dir}/seg_{i:02d}.mp4"
        run(f'ffmpeg -y -ss {s} -to {e} -i "{VIDEO}" -c copy "{p}" 2>/dev/null')
        parts.append(p)
    list_path=f"{clip_dir}/list.txt"
    with open(list_path,'w') as f:
        f.write('\n'.join(f"file '{p}'" for p in parts))
    run(f'ffmpeg -y -f concat -safe 0 -i "{list_path}" -c copy "{merged_path}"')
    return merged_path

def burn_subs(input_path, srt_path, output_path):
    import shutil, hashlib
    tmp_srt=f"/tmp/sub_{hashlib.md5(srt_path.encode()).hexdigest()[:6]}.srt"
    shutil.copy(srt_path, tmp_srt)
    style="FontName=PingFang SC,FontSize=16,PrimaryColour=&HFFFFFF,BorderStyle=1,Outline=2,Shadow=1,Alignment=2,MarginV=40"
    cmd=(f'ffmpeg -y -i "{input_path}" '
         f'-vf "subtitles={tmp_srt}:force_style=\'{style}\'" '
         f'-c:v libx264 -crf 18 -preset fast -c:a copy "{output_path}"')
    ok=run(cmd)
    if ok: print(f"  ✓ 字幕烧录完成")

# ── Per-clip worker (runs in separate process) ────────────
def process_clip(args):
    key, cfg, ja_entries, zh_entries = args
    title    = cfg["title"]
    segments = cfg["segments"]
    clip_dir  = f"{OUT}/{key}"
    video_dir = f"{clip_dir}/视频"
    cover_dir = f"{clip_dir}/封面"
    for d in [clip_dir, video_dir, cover_dir]:
        os.makedirs(d, exist_ok=True)

    total_sec = sum(e-s for s,e in segments)
    mm, ss = divmod(int(total_sec), 60)
    print(f"[{key}] {title}  时长 {mm}:{ss:02d}  ({len(segments)} 段)", flush=True)

    merged  = f"{video_dir}/merged.mp4"
    srt_out = f"{clip_dir}/bilingual.srt"
    final   = f"{video_dir}/hakuba_{key}_final.mp4"

    print(f"  [{key}] 切片 + 合并...", flush=True)
    cut_and_merge(segments, merged, clip_dir)
    print(f"  [{key}] 生成双语字幕...", flush=True)
    make_bilingual_srt(segments, ja_entries, zh_entries, srt_out)
    print(f"  [{key}] 烧录字幕...", flush=True)
    burn_subs(merged, srt_out, final)
    print(f"  [{key}] 取封面帧...", flush=True)
    for t in [5, 30, 60, 100, 150]:
        frame_path = f"{cover_dir}/frame_{t:03d}s.png"
        run(f'ffmpeg -y -ss {t} -i "{merged}" -vframes 1 "{frame_path}" 2>/dev/null')
    print(f"  [{key}] ✓ → {clip_dir}/\n", flush=True)
    return key

# ── Main ─────────────────────────────────────────────────
if __name__ == "__main__":
    from concurrent.futures import ProcessPoolExecutor, as_completed

    print("加载字幕文件...")
    ja_entries = parse_srt(JA_SRT)
    zh_entries = parse_srt(ZH_SRT)
    print(f"  日文: {len(ja_entries)} 条  中文: {len(zh_entries)} 条\n")

    tasks = [(key, cfg, ja_entries, zh_entries) for key, cfg in CLIPS.items()]
    workers = min(len(tasks), 4)
    print(f"并行处理 {len(tasks)} 条（{workers} 进程）...\n")

    with ProcessPoolExecutor(max_workers=workers) as ex:
        futs = {ex.submit(process_clip, t): t[0] for t in tasks}
        for f in as_completed(futs):
            key = futs[f]
            try:
                f.result()
            except Exception as e:
                print(f"  ✗ [{key}] 出错: {e}")

    print("全部完成 ✓")
